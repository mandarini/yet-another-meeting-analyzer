...
-- Drop all existing policies on user_roles more thoroughly
DO $$ 
DECLARE
    policy_name text;
BEGIN
...
-- Drop any existing policies on role_management
DO $$ 
DECLARE
    policy_name text;
BEGIN
    FOR policy_name IN 
        SELECT policyname 
        FROM pg_policies 
        WHERE tablename = 'role_management'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON role_management', policy_name);
    END LOOP;
END $$;
...
DROP TRIGGER IF EXISTS sync_role_management_trigger ON user_roles;
